...
1. Use the [delivery index](../../delivery/README.md) to select the governing
   specification and tracking Issue. Read only relevant specification sections.
2. Use the [ADR index](../../adr/README.md) to select only decisions implicated
   by the requested boundary.
3. Read the nearest area README only after affected files or deployable areas
   are known.
4. Run the bounded checks in [live state](../reference/live-state.md), then
   return here.
...
- For governance or other public documentation, read
  [public safety](../reference/public-safety.md), return here, then continue.
