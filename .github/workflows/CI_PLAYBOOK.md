...
| Browser E2E locators | Accessible-name substring matching can resolve both a landmark and a control whose names share the same words. | Prefer exact accessible-name matching when the intended control has a reviewed label; keep strict locator mode as the uniqueness guard. |
